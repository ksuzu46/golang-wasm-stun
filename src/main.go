...
func createStunRequest(_ js.Value, _ []js.Value) interface{} {
	...
	if err != nil {
		...
		panic(fmt.Sprintln("STUN dial faild:", err.Error()))
	}
	...
	if err := c.Do(message, func(res stun.Event) {
		if res.Error != nil {
			...
			dom.AddClass("result", "error")
			panic(fmt.Sprintln("res.Error:", err.Error()))
		}
		...
		if err := xorAddr.GetFrom(res.Message); err != nil {
			...
			dom.AddClass("result", "error")
			panic(fmt.Sprintln("xorAddr FAILED!!:", err.Error()))
		}
		...
		dom.AddClass("result", "error")
		panic(fmt.Sprintln("ERROR in c.Do()!!:", err.Error()))
	}
	return nil
}
